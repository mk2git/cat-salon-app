...
const events = {!! $events !!};
const reserved_status = @json(config('reserve_create.reserved'));
...
function showCalendar(year, month) {
  for ( i = 0; i < config.show; i++) {
    ...
    const calendarHtml = createCalendar(year, month, events, reserved_status)
    // ここからDOM操作
...
function createCalendar(year, month, events, reserved_status) {
    ...
    for (let d = 0; d < 7; d++) {
      if (w == 0 && d < startDay) {
        ...
        if (dayCount == tDate && month == tMonth && year == tYear) {
          ...
          calendarHtml += '<td class = "w-12 h-12 border" id="' + unixTime + '"><span class="font-bold underline decoration-double decoration-red-600">' + dayCount + '</span>';
